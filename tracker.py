...
class Tracker:
    ...
    def get_stable_id(self, track_id):
        """Map fluctuating ByteTrack IDs to stable IDs."""
        if track_id not in self.id_map:
            self.id_map[track_id] = self.next_stable_id
            self.next_stable_id += 1
        return self.id_map[track_id]
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        for frame_num, detection in enumerate(detections):
            ...
            # Reclassify goalkeepers as players
            ...
            for det in detection_with_tracks:
                ...
                if cls_id == cls_names_inv["player"]:
                    stable_id = self.get_stable_id(track_id)
                    tracks["players"][frame_num][stable_id] = {"bbox": bbox}

                elif cls_id == cls_names_inv["referee"]:
                    tracks["referees"][frame_num][track_id] = {"bbox": bbox}

            # Process balls separately (not tracked)
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control):
        ...
        team_1 = team_1_num_frames / (team_1_num_frames + team_2_num_frames) if (team_1_num_frames + team_2_num_frames) > 0 else 0
        team_2 = team_2_num_frames / (team_1_num_frames + team_2_num_frames) if (team_1_num_frames + team_2_num_frames) > 0 else 0
